...
function scoreMarkdownFile(file) {
  const relativePath = String(file?.relativePath ?? '').toLowerCase();
  const name = String(file?.name ?? '').toLowerCase();
  let score = 40;

  if (name === 'agents.md') score += 180;
  if (name === 'heartbeat.md') score += 160;
  if (name === 'readme.md') score += 120;
  if (['memory.md', 'system.md', 'profile.md', 'mission.md', 'prompt.md'].includes(name)) score += 110;
  if (/(todo|task|tasks|plan|roadmap|strategy|rule|rules|guide|manual|policy)/.test(name)) score += 90;
  if (relativePath.includes('/skills/')) score += 60;
  if (relativePath.includes('/agent/') || relativePath.includes('/openclaw/')) score += 20;
  if (/(daily|journal|dialog|conversation|chat|meeting|record|transcript|summary|report|note|notes)/.test(relativePath)) score -= 35;
  if (/(archive|bak|backup|tmp|draft|old|history)/.test(relativePath)) score -= 40;
  if (/\b20\d{2}[-_/.]?\d{1,2}[-_/.]?\d{1,2}\b/.test(relativePath)) score -= 24;

  const ageMs = Date.now() - (Number(file?.mtimeMs ?? 0) || 0);
  if (ageMs < 3 * 24 * 60 * 60 * 1000) score += 16;
  else if (ageMs < 14 * 24 * 60 * 60 * 1000) score += 8;

  const size = Number(file?.size ?? 0) || 0;
  if (size > 400000) score -= 18;
  else if (size > 120000) score -= 8;

  const importance = score >= 150 ? '核心' : score >= 95 ? '重点' : score >= 55 ? '常用' : '其他';
  return {
    score,
    importance,
  };
...
async function buildAgentSnapshot(config, agent, globalContext) {
  ...
  const markdownFiles = sortMarkdownFiles(await listMarkdownFiles(agent.workspace));
